...
import { updateTrip } from "@/utils/trip/utility";
...
export default function EditTripPage({
  ...
  React.useEffect(() => {
    ...
    if (tripResData?.driverId !== userData?.id) {
  ...
  const [price, setPrice] = React.useState<number>(tripResData?.price || 0);
  const [description, setDescription] = React.useState(
    tripResData?.description || ""
  );
  ...
  const onSubmit = (e: any) => {
    ...
    const data = {
      ...
      endTime: calculateApproximateEndTime(
        startTime,
        tripResData?.distance
      )?.toISOString(),
      price,
    ...
    updateTrip(tripResData?.id, result.data);
        ...
        <header className="sticky top-0 z-10 flex h-[57px] items-center gap-1 border-b bg-background px-4">
          <h1 className="text-xl font-semibold">
            <div className="flex space-x-2 items-center pl-2">
              ...
              <span>
                Редагування поїздки (Поїздка на RideTogether #{tripResData.id})
              </span>
            </div>
          </h1>
          <Button
              ...
              <fieldset className="grid gap-6 rounded-lg border p-4">
                ...
                <div className="grid gap-3">
                  ...
                  <Textarea
                    ...
                    defaultValue={tripResData?.description}
                  />
                </div>

                <div className="grid gap-3">
                  <Label className="text-muted-foreground font-light italic">
                    * Редагуючи інформацію, що стосується поїздки, ви НЕ можете
                    змінити початкову та кінцеву точки. Для того, щоб змінити ці
                    значення - вам потрібно перестворити поїздку з новими
                    точками.
                  </Label>
                </div>
                ...
                <AlertDialog>
                  ...
                  <AlertDialogContent>
                    ...
                    <AlertDialogFooter>
                      <AlertDialogCancel>Ні, скасувати</AlertDialogCancel>
...
function calculateApproximateEndTime(
  startTime: Date,
  approximateDistance: number
) {
  const averageCarSpeedKph = 75.0;
  const timeHours = approximateDistance / averageCarSpeedKph;
  const endTime = new Date(startTime.getTime() + timeHours * 3600000);
  return endTime;
}
